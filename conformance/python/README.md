...
```text
service=runtime tier=surface spec=spec/v1@v0.2.2
...
```bash
pipx install "arp-conformance==0.2.2"
arp-conformance --version
python -c "import arp_conformance; print(arp_conformance.SPEC_REF)"
```
...
`arp_conformance.SPEC_REF` exposes the spec tag used by the package (for example, `spec/v1@v0.2.2`).
