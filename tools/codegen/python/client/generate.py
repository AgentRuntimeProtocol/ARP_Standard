...
def generate_facade(*, service: str, bundled_openapi: dict[str, Any], output_dir: Path) -> None:
    ...
    client_lines.append("        *,")
    client_lines.append("        client: _LowLevelClient | None = None,")
    ...
    client_lines.append("                headers={} if headers is None else dict(headers),")
    client_lines.append("                cookies={} if cookies is None else dict(cookies),")
    ...
    client_lines.append("                httpx_args={} if httpx_args is None else dict(httpx_args),")
    client_lines.append("            )")
    client_lines.append("        self._client = client")
    client_lines.append("")
    client_lines.append("    @property")
    client_lines.append("    def raw_client(self) -> _LowLevelClient:")
