...
def main() -> int:
    ...
    if args.clean:
        for target in (model_dist_dir, client_dist_dir, server_dist_dir):
    ...
    for path in [*model_dists, *client_dists, *server_dists]:
        print(f"- {path.relative_to(repo_root)}")

    if args.list_wheel:
        for wheel in [*model_dists, *client_dists, *server_dists]:
            if wheel.suffix != ".whl":
                continue
